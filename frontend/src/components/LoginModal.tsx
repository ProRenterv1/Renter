import { FormEvent, memo, useCallback, useEffect, useState, type Ref } from "react";
...
GoogleButtonSlot.displayName = "GoogleButtonSlot";
...
export function LoginModal({
  ...
  const setGoogleButtonRef = useCallback((node: HTMLDivElement | null) => {
    setGoogleButtonEl(node);
  }, []);
  ...
  useEffect(() => {
    ...
    if (isGisDebugEnabled()) {
      console.debug("[GIS] modal opened");
    }
  }, [open]);
  ...
  const handleGoogleCredential = async (response: GoogleCredentialResponse) => {
    ...
    setLoginError(null);
    try {
      const tokens = await authAPI.googleLogin({ id_token: credential });
  ...
  useEffect(() => {
    setCredentialCallback((response) => {
      void handleGoogleCredential(response);
    });
  }, [handleGoogleCredential, setCredentialCallback]);

  useEffect(() => {
    const debugEnabled = isGisDebugEnabled();
    if (debugEnabled) {
      console.debug("[GIS] render effect", {
        open,
        ready: googleReady,
        clientId: Boolean(googleClientId),
      });
    }
    if (isSignUp || !open || !googleReady || !googleClientId || !window.google?.accounts?.id) {
      return;
    }

    const container = googleButtonEl;
    if (debugEnabled) {
      console.debug("[GIS] container check", { exists: Boolean(container) });
    }
    if (!container) {
      return;
    }
    ...
    const render = (reason: string) => {
      ...
      if (debugEnabled) {
        console.debug("[GIS] render attempt", { reason, width, height });
      }
      if (width <= 0 || height <= 0) {
        return;
      }
      if (hasRendered && width === lastWidth && height === lastHeight) {
        return;
      }
    ...
    const scheduleRender = (reason: string) => {
      ...
      rafId = window.requestAnimationFrame(() => {
        rafIdNext = window.requestAnimationFrame(() => {
          scheduled = false;
          render(reason);
        });
      });
    ...
    return () => {
      ...
      if (rafIdNext) {
        window.cancelAnimationFrame(rafIdNext);
      }
      resizeObserver.disconnect();
    };
  }, [open, googleClientId, googleReady, googleButtonEl, isSignUp]);
